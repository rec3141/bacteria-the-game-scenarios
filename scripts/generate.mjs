...
// The scenario ENVELOPE — the shape of the outermost object, as opposed to its contents. Kept here as
// data because three separate things need to agree on it: the tool schema the model is handed, the
// repair below, and the retry message. The validator owns the authoritative copy; these are the keys
// the MODEL is responsible for, plus the two we stamp ourselves.
const TOP_LEVEL = ["schema", "version", "meta", "env", "resources", "particles", "actions", "organisms", "column", "terrain"];
const META_KEYS = ["title", "date", "lesson", "citation", "difficulty", "realWorldBasis"];
...
// The tool's own schema. This used to be `{ type: "object", additionalProperties: true }` — literally
// "any object at all" — with the real shape described only in prose at the top of a 34KB prompt. On
// 2026-07-24 and -25 the model started wrapping its answer as {"scenario": {...}}, the retry unwrapped
// it too far and hoisted the meta fields to the top level, and both the daily cron and a player's DOI
// request died on "unknown top-level field". Naming the properties here puts the envelope in the same
// channel as the answer instead of 300 lines upstream of it. Contents stay loose on purpose: the
// PARAMETER TABLE is the contract for those, and validateScenario is what enforces it.
const SCENARIO_TOOL_SCHEMA = {
  type: "object",
  properties: {
    schema: { type: "string", description: 'always "bacteria-scenario"' },
    version: { type: "integer", description: "always 1" },
    meta: { type: "object", description: `the descriptive fields — ${META_KEYS.join(", ")}. They belong HERE, not at the top level.` },
    env: { type: "object", description: "dotted CFG paths from the PARAMETER TABLE, each a number" },
    resources: { type: "array" },
    particles: { type: "object" },
    actions: { type: "object" },
    organisms: { type: "object" },
    column: { type: "object" },
    terrain: { type: "array" },
  },
  required: ["meta", "env"],
...
// Repair the two envelope mistakes a model actually makes. Both are shape, not content: the scenario it
// designed is fine, it just handed it over in the wrong box. The code already takes the position that
// the envelope is OURS — it stamps schema/version rather than letting a fumbled discriminator fail an
// otherwise-good level — and these are the same class of thing. Anything beyond re-boxing (a bad
// parameter, an invented primitive, an out-of-range value) is still the model's to get right and still
// fails the run.
function unwrapEnvelope(obj) {
  if (!obj || typeof obj !== "object" || Array.isArray(obj)) return obj;
  const keys = Object.keys(obj);
  // 1. wrapped in a container of its own invention: {"scenario": {...}}, {"result": {...}}
  if (keys.length === 1 && !TOP_LEVEL.includes(keys[0])) {
    const inner = obj[keys[0]];
    if (inner && typeof inner === "object" && !Array.isArray(inner)) return unwrapEnvelope(inner);
  }
  // 2. meta flattened INTO the top level: {"title": ..., "lesson": ..., "env": {...}} with no "meta".
  //    This is what the retry produced both days — told "scenario" was unknown, it unwrapped one level
  //    too many. Only fires when meta is absent, so a scenario that got it right is never touched.
  if (!obj.meta && META_KEYS.some((k) => k in obj)) {
    const meta = {}, rest = {};
    for (const [k, v] of Object.entries(obj)) (META_KEYS.includes(k) ? meta : rest)[k] = v;
    return { ...rest, meta };
  }
  return obj;
}
...
// Importable so selftest can drive unwrapEnvelope directly. Running the generator for real writes into
// scenarios/ and rebuilds index.json, which is not something a test suite should do to the library, so
// the CLI body below runs only when this file IS the entry point.
export { unwrapEnvelope, TOP_LEVEL, META_KEYS, SCENARIO_TOOL_SCHEMA };
...
(async () => {
  ...
  // failed, and a manual dispatch can never clobber a scenario people are already playing.
  if (mode === "daily" && existsSync(join(scenDir, `${slug(`daily-${today}`)}.json`))) {
    console.log(`daily-${today} already exists — nothing to do`);
    return;
  }
  ...
  if (!result.ok) {
    ...
    // Say what the envelope IS, not just which key was wrong. Bare "unknown top-level field
    // \"scenario\"" is what made the retry unwrap one level too far and hoist meta to the top.
    ({ raw, result } = await generateOnce(`${prompt}\n\nYour previous attempt was REJECTED: ${result.reason}.\n` +
      `The top-level object may contain ONLY these keys: ${TOP_LEVEL.join(", ")}. The descriptive fields ` +
      `(${META_KEYS.join(", ")}) go INSIDE "meta" — they are not top-level keys — and the scenario must not ` +
      `be nested inside a wrapper object of any name. Fix exactly what was rejected and output the corrected JSON only.`));
  }
